...
def parse_header(file_path, target_arch):
    ...
    type_keywords = {
        'int', 'char', 'short', 'long', '__int64', 'float', 'double', 'void', 
        '__m128', '__m128i', '__m128d', '__m256', '__m256i', '__m256d', 
        '__m512', '__m512i', '__m512d', 'size_t', 'uint8_t', 'uint16_t', 
    ...
    for line in lines:
        ...
        if target_arch == 'arm64' and category == 'x86_64': continue
        if target_arch == 'x86_64' and category == 'arm64': continue
                    ...
                    if raw_arg.endswith('*') or raw_arg.endswith('&'):
                        ...
                        if word_match:
                            ...
                            if last_word in type_keywords:
                                ...
                                v_name = last_word
                                if not t_str: t_str = "int" # Fallback if parsing fails
                        else:
                            t_str, v_name = raw_arg, f"arg{i}"
        ...
        # Add to dictionary
        intrinsics[category][name] = {
            "proto": proto,
            "call": " ".join(call_pieces)
        }
...
def main():
    parser = argparse.ArgumentParser(description="Parse MSVC headers and generate test calls.")
